...
struct NodeRstar {
    ...
    std::shared_ptr<std::vector<std::pair<NodeRstar, double>> > predecessors_nodes; // predecessors_nodes with C_low(path_s'_s)
    ...
    NodeRstar() {
        ...
        g = 0;
        bp = nullptr;
        predecessors_nodes = std::make_shared<std::vector<std::pair<NodeRstar, double>>>();
        path_to_bp = nullptr;
        C_low = 0;
    }

    NodeRstar(int i_, int j_, int avoid_, double F_, double g_, std::shared_ptr<NodeRstar> bp_,
              std::shared_ptr<std::vector<std::pair<NodeRstar, double>>> predecessors_nodes_,
